...
class AltBtreeBuffer {
 ...
 public:
  ...
  bool update(const key_t &key, const val_t &val);
  void insert(const key_t &key, const val_t &val);
  bool remove(const key_t &key);
  size_t scan(const key_t &key_begin, const size_t n,
                     std::vector<std::pair<key_t, val_t>> &result);
  void range_scan(const key_t &key_begin, const key_t &key_end,
                         std::vector<std::pair<key_t, val_t>> &result);
 ...
 private:
  ...
  internal_t *allocate_internal();
  leaf_t *allocate_leaf();
  size_t available_node_index();
